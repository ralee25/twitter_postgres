...
for file in $files; do
    ...
    ./load_tweets.py --db "postgresql://postgres:pass@localhost:1444" --inputs "$file"
done

echo 'load denormalized'
for file in $files; do
    # use SQL's COPY command to load data into pg_denormalized
    unzip -p "$file" | tr -d '\000' | psql "postgresql://postgres:pass@localhost:1433" -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
